...
public class StubDatabase implements DataAccessI {
    ...
    public boolean updateBankAccount(BankAccount toUpdate, BankAccount newAccount) {
        ...
        if (index >= 0 && toUpdate.equals(newAccount))
        {
            if (toUpdate.getAccountName().equals(newAccount.getAccountName()))
            {
                return false;
            }
     ...
     *
     * @param toUpdate transaction that need to be updated
     * @param newTransaction transaction which will be set
     * @return true if updated successfully.
     */
    public boolean updateTransaction(Transaction toUpdate, Transaction newTransaction) {
        boolean toReturn = false;

        if (!toUpdate.equals(newTransaction)) {
            while(transactions.contains(toUpdate)) {
                int index = transactions.indexOf(toUpdate);
                if (index >= 0) {
